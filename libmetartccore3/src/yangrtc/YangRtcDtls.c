...
void ssl_on_info(const SSL *ssl, int32_t where, int32_t ret) {
	YangRtcDtls* dtls = (YangRtcDtls*)SSL_get_ex_data(ssl, 0);
	const char *method;
...
int32_t yang_run_rtcdtls_app(YangRtcDtls *dtls) {
	...
	for (int32_t i = 0; arq_count < arq_max_retry && i < max_loop; i++) {
		...
		if (size > 0&& (err = yang_rtc_sendData(dtls->udp,(char*)data, size)) != Yang_Ok) {
...
int32_t yang_doHandshake(YangRtcDtls *dtls) {
	...
	if (size > 0)	yang_rtc_sendData(dtls->udp, (char*) data, size);
...
int32_t yang_decodeHandshake(YangRtcDtls *dtls, char *data, int32_t nb_data) {
    yang_trace("\nDTLS yang_decodeHandshake\n");
	   /*uint16_t size = 0;
	    ...
	    yang_trace("\ndtls data len=%d,size=%d,ht=%u",nb_data,size,handshake_type);
*/
...
int32_t yang_get_srtp_key(YangRtcDtls *dtls, char *precv_key, int *precvkeylen,
